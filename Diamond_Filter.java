...
public class Diamond_Filter implements PlugIn {
	...
	private class DiamondFilterDialog extends Dialog
	implements ActionListener, TextListener, Runnable {
		public DiamondFilterDialog() {
			super(WindowManager.getFrame("Results"), "Diamond Filter setup", true);
		...
		public ScheduledFuture<Object> scheduleFeedbackTask() {
			return scheduler.schedule(Executors.callable(this),
					EVAL_DELAY_MS, TimeUnit.MILLISECONDS);
		}
		...
		public void run() {
			if (isValidResult(exprArea.getText())) {
				...
				/* on error, we schedule yet another deferred task
				 * it will show the error after another period of time
				 */
				feedbackTask =
					scheduler.schedule(Executors.callable(
							new Runnable() {
								public void run() {
									feedbackArea.setText(evaluate(exprArea.getText()));
								}	
	...
	private static class ExpressionHelperPanel extends Panel
		...
		private String getSubExpr() {
			return functionChooser.getSelectedItem() + "(" +
				ColumnIdToken.makeColExpr(columnChooser.getSelectedItem()) + ")";
		}
	...
	protected String evaluate(String expr) {
		...
		try {
			...
			try {
				...
				if (!tokens.isEmpty()) {
					throw new ParseException("There is extra code at the end of the line!");
				}
	...
	protected double getFactor(Queue<BaseToken> q) throws ParseException {
		...
		if (nextToken == null) {
			throw new ParseException("Expecting more, but ran into the end of the line!");
		}
		...
		case Literal:
			try {
				...
				throw new ParseException("Found a literal but did not have LiteralToken, contact developer.");
			}
		case Aggregator:
			try {
				...
				throw new ParseException("Found an aggregator but did not have AggregatorToken, contact developer.");
			}
		case LParen:
			q.remove();
	...
	protected float[] getColumn(Queue<BaseToken> q) throws ParseException {
		...
		case ColumnID:
			try {
				...
				throw new ParseException("Found a column ID, but did not have ColumnIDToken, contact developer.");
			}
		default:
			throw new ParseException("Expecting a table column ID, but found something else!");
		}
		return ret;
	}
	...
	protected void getRParen(Queue<BaseToken> q) throws ParseException {
		...
		if ((nextToken == null) || (nextToken.getType() != TokenType.RParen)) {
			throw new ParseException("Mismatched parentheses! Expected right parenthesis.");
		}
	}
	
	protected static Queue<BaseToken> tokenize(String expr) throws TokenizeException {
        ...
        while (st.ttype != StreamTokenizer.TT_EOF) {
        	...
        	case StreamTokenizer.TT_EOF:
        		throw new RuntimeException("Checking for EOF in tokenizer done incorrectly, contact developer.");
        	case StreamTokenizer.TT_EOL:
        		throw new RuntimeException("Checking for EOL in tokenizer done incorrectly, contact developer.");
        	case QUOTE_CHAR:
        	...
        	default:
        		...
        		if (ttype == null) {
        			throw new TokenizeException("Invalid character: " + (char)st.ttype);
        		}
	...
	protected static BaseToken getNewAggToken(String tokenVal) throws TokenizeException {
	...
	protected static void getNextToken(StreamTokenizer st) {
		try {
        	...
        	e.printStackTrace();
        	throw new RuntimeException("Received IO Exception from StringReader, contact developer.");
        }		
	}
	...
	protected static enum TokenType {
		...
		private static final Map<Character, TokenType>
			charToToken = new HashMap<Character, TokenType>();
	...
	protected static enum AggregatorType {
		...
		SUM(new SumEngine()),
		COUNT(new CountEngine()),
		AVERAGE(new AvgEngine()),
		PRODUCT(new ProdEngine()),
		MIN(new MinEngine()),
		MAX(new MaxEngine());
