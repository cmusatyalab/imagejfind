...
{
  GtkWidget *imagejfind;

  gtk_init(&argc,&argv);
  g_xml = glade_xml_new(IMAGEJFIND_GLADEDIR "/imagejfind.glade",NULL,NULL);
  g_assert(g_xml != NULL);

  glade_xml_signal_autoconnect(g_xml);
  imagejfind = glade_xml_get_widget(g_xml,"imagejfind");
  ...
  gtk_widget_show_all(imagejfind);
