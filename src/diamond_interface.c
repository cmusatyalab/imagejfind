...
static ls_search_handle_t generic_search (char *filter_spec_name) {
  ...
  // append our stuff
  f1 = g_open(IMAGEJFIND_FILTERDIR "/rgb-filter.txt", O_RDONLY);
  if (f1 == -1) {
    perror("can't open " IMAGEJFIND_FILTERDIR "/rgb-filter.txt");
    return NULL;
  }
...
ls_search_handle_t diamond_imagej_search(gdouble threshold, const gchar *macro_file) {
  ...
  err = ls_add_filter_file(dr, DEV_ISA_IA32,
			   IMAGEJFIND_FILTERDIR "/fil_imagej_exec.so");
  g_assert(!err);

  err = ls_set_blob(dr, "imagej_exec", blob_len, blob_data);
  g_assert(!err);
